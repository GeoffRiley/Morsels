...
class LoopHelperTests(unittest.TestCase):
    ...
    def test_next_and_is_last(self):
        ...
        # Starting with None shouldn't be a problem
        response2 = loop_helper([None, None, 'a', 'b', None, None])
        item, info = next(response2)
        self.assertEqual(item, None)
        self.assertEqual(info.next, None)
        item, info = next(response2)
        self.assertEqual(item, None)
        self.assertEqual(info.next, 'a')

        # Ending without None works too
        response3 = loop_helper([1, 2, 3, 4])
        a, info = next(response3)
        b, info = next(response3)
        c, info = next(response3)
        d, info = next(response3)
